...
<div class="w-full max-w-md bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl p-8 space-y-6">
    <div class="text-center space-y-2">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Login ASTALA</h1>
        <p class="text-xs text-gray-500 dark:text-gray-400">Sistem Manajemen Aset & Inventaris PT Lintasarta</p>
    </div>

    <?php if (session()->getFlashdata('success')): ?>
        <div class="p-3 bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/30 text-green-700 dark:text-green-400 rounded-xl text-xs">
    ...
    <form method="post" action="<?= site_url('auth/login') ?>" class="space-y-4">
        ...
        <div>
            ...
            <input type="email" name="email" value="<?= esc(old('email')) ?>" required autofocus placeholder="nama@lintasarta.co.id"
        ...
        <div class="text-center pt-2">
            <a href="<?= site_url('auth/signup') ?>" class="text-xs text-blue-600 hover:text-blue-700 dark:text-blue-400 font-medium">
